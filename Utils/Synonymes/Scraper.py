...
if os.path.isfile('./synonymes/synonymes.json'):
    with open('./synonymes/synonymes.json',"rb") as syn:
        dico = json.load(syn)
else:
    dico = {}


with open("./words.json","rb") as words:
...
open(f'./synonymes/synonymes.json', 'w+').write(json.dumps(dico))
